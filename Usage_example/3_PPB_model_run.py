...
def return_list_of_closest(p_idx, particlesDF, L, np=30):
    ...
    res = []
    p1 = [
        particlesDF.iloc[p_idx]["xPos"],
        particlesDF.iloc[p_idx]["yPos"],
        particlesDF.iloc[p_idx]["zPos"],
    ]
...
if __name__ == "__main__":
    ...
    # Initialize main input and output data
    ...
    if load_res_flag != True:
        for idx, row in tqdm(particlesDF.iterrows()):
            ...
            neig_idx_list = return_list_of_closest(
                idx, particlesDF, L_domain, np=no_of_np
            )

            inp_part = []
            for neig in neig_idx_list:
                inp_part.append(
                    calc_relative_distance(
                        particlesDF.iloc[idx]["xPos"],
                        particlesDF.iloc[neig]["xPos"],
                        L_domain,
                    )
                )
                inp_part.append(
                    calc_relative_distance(
                        particlesDF.iloc[idx]["yPos"],
                        particlesDF.iloc[neig]["yPos"],
                        L_domain,
                    )
                )
                inp_part.append(
                    calc_relative_distance(
                        particlesDF.iloc[idx]["zPos"],
                        particlesDF.iloc[neig]["zPos"],
                        L_domain,
                    )
                )
    ...
    plt.plot(
        [res["Fx_real"].min(), res["Fx_real"].max()],
        [res["Fx_real"].min(), res["Fx_real"].max()],
    )
